...
func (b *bot) fetchLatest() {
	var count int
	if err := b.opr.DB.Model(&cherry.PullRequest{}).Where("owner = ? AND repo = ?",
...
func (b *bot) fetchDuration(d time.Duration) {
	...
	if err := b.opr.DB.Model(&cherry.PullRequest{}).Where("owner = ? AND repo = ? AND created_at < ?",
...
func (b *bot) fetchHistory(startID int) {
	...
	for prList := range ch {
		for _, pr := range prList {
			if err := b.createPullRequest(pr); err != nil {
